import type { BaseGridPlugin } from ".";
import type {
  GridCellEventPayloadPlugin,
  GridEventPayloadPlugin,
  GridPluginEvent,
} from "../../../../types/grid-plugin";

export const pluginEvents: Array<GridPluginEvent> = [
  {
    type: "click",
    element: "cell" as const,
    name: "gridLink",
    method(this: BaseGridPlugin, { data: [headId, rowId, event] }: GridCellEventPayloadPlugin<MouseEvent>) {
      gridLinkClick(this, headId, rowId, event);
    },
  },
  {
    type: "applyEdit",
    name: "applyEdit",
    method(this: BaseGridPlugin, { data: [eventData] }: GridEventPayloadPlugin<CustomEvent>) {
      const { headId, rowId, value } = eventData.detail;
      applyEdit(this, headId, rowId, value);
    },
  },
];

// eslint-disable-next-line complexity
async function gridLinkClick(plugin: BaseGridPlugin, headId: string, rowId: string, event: MouseEvent) {
  const { rows, head, view } = plugin.grid;

  const target = event.target as HTMLElement;
  const activeCell = view.activeCell;
  if (activeCell?.classList.contains("aras-grid-active-cell_editing")) return;

  const headInfo = head.get(headId);
  const initialRowInfo = rows.get(rowId);
  const metadata = plugin.grid.getCellMetadata(headId, rowId);

  if (!headInfo || !initialRowInfo || !metadata) return;

  const linkPropertyId = headInfo.linkProperty ? initialRowInfo[headInfo.linkProperty] : undefined;
  const rowInfo = linkPropertyId ? rows.get(linkPropertyId) || {} : initialRowInfo;

  const propertyName = headInfo.name || headId;
  const currentValue = rowInfo[propertyName];
  const sourceItemTypeName = rowInfo[`${propertyName}@aras.type`] || metadata.sourceItemTypeName;

  if (target.classList?.contains("aras-grid-link")) {
    aras.uiShowItem(sourceItemTypeName, currentValue);
    return;
  }

  const fileIcon = target.closest(".aras-grid-file-icon");
  const isSelectFileIcon = target.closest(".aras-grid-file-icon_select-file");

  if (fileIcon && !isSelectFileIcon) {
    throw new Error("File viewing is not implemented in ArasJS yet.");
  }
}

function applyEdit(plugin: BaseGridPlugin, headId: string, rowId: string, value: string) {
  const cellMetadata = plugin.grid.getCellMetadata(headId, rowId);
  const { dataType, sourceItemTypeName } = cellMetadata;
  if (dataType !== "item" || typeof value !== "string") return;

  const itemNode = aras.uiGetItemByKeyedName(sourceItemTypeName, value, true);
  if (!itemNode) return;

  plugin.grid.rows.set(rowId, {
    ...plugin.grid.rows.get(rowId),
    [`${headId}`]: itemNode.getAttribute("id"),
    [`${headId}@aras.type`]: sourceItemTypeName,
    [`${headId}@aras.keyed_name`]: value,
  });
}
